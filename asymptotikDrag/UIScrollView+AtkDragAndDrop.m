...
- (void)autoScrollDragMoved:(CGPoint)point
{
   ...
   // NSLog(@"auto scroll delta x=%f y=%f",autoScrollDelta.x,autoScrollDelta.y);
    if(!CGPointEqualToPoint(autoScrollDelta, CGPointZero))
    {
        ...
        if(autoScrollTimer == nil)
            self.autoScrollTimer = [NSTimer scheduledTimerWithTimeInterval:0.001 target:self selector:@selector(autoScrollTimerTick:) userInfo:nil repeats:YES];
    }
